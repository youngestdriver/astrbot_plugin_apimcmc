...
- 🚀 **自动启动**: 可配置插件加载时自动启动监控（延迟5秒启动，首次为静默初始化）
...
1. **首次启动监控**
   - 启动任务后先静默获取一次服务器状态并写入缓存
   - 首次启动不发送任何“监控已启动”通知
   - 等待一个 `check_interval` 周期后，才开始按变化规则推送消息
...
使用 `/重置监控` 可以清除缓存；下次检测会先静默重建缓存，再从下一轮开始检测变化。
...
启动后行为说明：
- 监控任务会先立即静默拉取一次状态作为基线
- 随后等待 `check_interval` 秒，再进行下一次检测与变化通知判断
...
- ⚠️ 监控任务首次启动不会推送“启动成功”消息，属于静默初始化
...
**Q: 为什么刚启动监控没有立刻收到通知？**
A: 这是预期行为。插件会先静默记录一次当前状态作为基线，等待下一个 `check_interval` 周期后再按变化情况通知。
